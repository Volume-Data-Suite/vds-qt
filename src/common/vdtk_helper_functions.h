...
inline const VDTK::VolumeSize QVector3DToVolumeSize(const QVector3D size) {
    return VDTK::VolumeSize(static_cast<uint32_t>(size.x()), static_cast<uint32_t>(size.y()),
...
inline const QVector3D VolumeSizetoQVector3D(const VDTK::VolumeSize size) {
    return QVector3D(static_cast<float>(size.getX()), static_cast<float>(size.getY()),
                     static_cast<float>(size.getZ()));
}
