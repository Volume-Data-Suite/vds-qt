...
{
	const std::filesystem::path path = std::filesystem::path(m_textPathToFile->text().toStdString());
	const QVector3D size(m_textSizeX->text().toFloat(), m_textSizeY->text().toFloat(), m_textSizeZ->text().toFloat());
	const QVector3D spacing(m_textSpacingX->text().toFloat(), m_textSpacingY->text().toFloat(), m_textSpacingZ->text().toFloat());
...
void DialogImportRAW3D::onOKButtonClicked()
{
	...
	{
		QMessageBox msgBox(QMessageBox::Warning, "Incomplete RAW 3D Import!", "Some required fields are empty!");
		msgBox.exec();
	}
}
...
bool DialogImportRAW3D::checkCurrentInput()
{
	...
	if (m_textSizeX->text().isEmpty() || m_textSizeY->text().isEmpty() || m_textSizeZ->text().isEmpty())
	{
		return false;
	}
	if (m_textSpacingX->text().isEmpty() || m_textSpacingY->text().isEmpty() || m_textSpacingZ->text().isEmpty())
	{
		return false;
	}
	return true;
...
{
	const QString path = QFileDialog::getOpenFileName(nullptr, tr("Select File"), QDir::homePath(), tr("*.raw"));
	if (path.isEmpty())
	{
		// Open file dialog got cancelled by user. Do not overwrite current text in case user open file dialog a second time accidentally
		return;
	}
